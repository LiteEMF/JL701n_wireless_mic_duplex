...
const int config_mixer_ch_self_buf_en = 1;
...
const  int  WTGV2_STACK2BUF = 0;  //等于1时解码申请buf会加大760，栈会减小
...
int wma_mem_ext[(23920 + 3) / 4] SEC(.wma_mem);
int wma_mem_tws_ext[(29334 + 3) / 4] SEC(.wma_mem);
...
#endif
#if CONFIG_CPU_BR28
const  int  JLA_HW_FFT = 1; //br28置1，br30置0
#elif CONFIG_CPU_BR30
const  int  JLA_HW_FFT = 0; //br28置1，br30置0
#endif
const int  JLA_INT24bit_INOUT = 0;
const int JLA_SUPPORT_CH = WIRELESS_DECODE_CHANNEL_NUM;     //jla解码输入通道数 1：单声道输入， 2:双声道输入(br30可支持2)
const int JLA_DMS_VAL = WIRELESS_CODING_FRAME_LEN;      //单位ms, 【只支持 25,50,100】
...
#else
#if (WIRELESS_CODING_SAMPLERATE <= 8000)
const int JLA_DMS_FSINDEX = 0;
#elif(WIRELESS_CODING_SAMPLERATE <= 16000)
const int JLA_DMS_FSINDEX = 1;
#elif(WIRELESS_CODING_SAMPLERATE <= 24000)
const int JLA_DMS_FSINDEX = 2;
#elif(WIRELESS_CODING_SAMPLERATE <= 32000)
const int JLA_DMS_FSINDEX = 3;
#elif(WIRELESS_CODING_SAMPLERATE <= 48000)
const int JLA_DMS_FSINDEX = 4;
#endif
#endif//(WIRELESS_CODING_SAMPLERATE != WIRELESS_DECODE_SAMPLERATE)
...
const  int  voicechange_fft_PLATFORM = PLATFORM_PARM_SEL;
...
const int CONFIG_AUDIO_DAC_UNDERRUN_MODE = 1;

const int CONFIG_RESAMPLE_HW_SCHEDULE_MODE = 1;
/**
