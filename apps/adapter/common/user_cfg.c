...
#if TCFG_APP_BT_EN
    bt_max_pwr_set(app_var.rf_power, 5, 8, app_var.rf_power);
#endif
...
#if BOARD_TEST_EN
    ret = syscfg_read(BOARD_TEST_MAC, mac_buf2, 6);
    if (ret == 6) {
        printf("use board_test mac addr");
        if (0 != memcmp(mac_buf2, mac_buf_tmp, 6)) {
            memcpy(bt_cfg.mac_addr, mac_buf2, 6);
            memcpy(mac_buf, mac_buf2, 6);
        }
        put_buf(mac_buf, 6);
    }
#endif
