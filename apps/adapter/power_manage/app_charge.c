...
void charge_start_deal(void)
{
    ...
    u8 app = app_get_curr_status();
    if (app == APP_CHARGE_STATUS) {
    ...
    if (get_charge_poweron_en() == 0) {
        ...
        if (app != APP_CHARGE_STATUS) {
            ...
            if (cpu_reset_by_soft()) {
                log_info("charge_start_idle\n");
                charge_start();
            } else {
                log_info("cpu_reset_because soft\n");
                cpu_reset();
            }
            wdt_init(WDT_32S);
...
u8 charge_ldo5v_off_deal(void)
{
    ...
    if ((get_charge_poweron_en() == 0)) {
        ...
        if (chargestore_get_power_status()) {
            ...
            vbat_check_init();
#endif
            if ((app != APP_NORMAL_STATUS) && (app_var.goto_poweroff_flag == 0)) {
#if TCFG_SYS_LVD_EN
            ...
            log_info("1 ldo5v off,enter softpoweroff\n");
            power_set_soft_poweroff();
#endif //TCFG_CHARGE_OFF_POWERON_NE
#if TCFG_CHARGESTORE_ENABLE
        } else {
            log_info("2 ldo5v off,enter softpoweroff\n");
            if (app != APP_CHARGE_STATUS) {
...
extern void ldoin_wakeup_set_edge(u8 edge, u8 both_edge);
int app_charge_event_handler(struct device_event *dev)
{
    ...
    switch (dev->event) {
    case CHARGE_EVENT_SHUT_DOWN:
        log_info("CHARGE_EVENT_SHUT_DOWN\n");
        power_set_soft_poweroff();
        break;
    case CHARGE_EVENT_LOWPOWER_SHUT_DOWN:
        log_info("CHARGE_EVENT_LOWPOWER_SHUT_DOWN\n");
        //ldoin_wakeup_set_edge(0, 0); //改为只有上升沿唤醒k
        power_set_soft_poweroff();
        break;
