...
struct adc_platform_data adc_data = {
	...
	.mic_bias_keep = 0,
	/*差分输入使能*/
	.mic_diff = TCFG_AUDIO_MIC_MODE,

	.mic1_ldo_isel   = TCFG_AUDIO_ADC_LDO_SEL,
	/*MIC 是否省隔直电容：
	  0: 不省电容  1: 省电容 */
	.mic1_capless    = TCFG_MIC_CAPLESS_ENABLE,
	/*MIC内部上拉电阻挡位配置，影响MIC的偏置电压
	  21:1.18K  20:1.42K   19:1.55K   18:1.99K   17:2.2K   16:2.4K   15:2.6K    14:2.91K  13:3.05K   12:3.5K   11:3.73K
	  10:3.91K    9:4.41K   8:5.0K    7:5.6K    6:6K    5:6.5K    4:7K    3:7.6K    2:8.0K    1:8.5K        */
	.mic1_bias_res   = 18,
	/*MIC LDO电压档位设置,也会影响MIC的偏置电压0:1.5v  1:8v  2:2.1v  3:2.4v 4:2.7v 5:3.0 */
	.mic1_ldo_vsel  = 3,
	/*MIC电容隔直模式使用内部mic偏置(PB7)*/
	.mic1_bias_inside = 1,
	/*保持内部mic偏置输出*/
	.mic1_bias_keep = 0,
	/*差分输入使能*/
	.mic1_diff = TCFG_AUDIO_MIC1_MODE,
	// ladc 通道
...
extern void recheck_vm_mode(void);
	recheck_vm_mode();
#endif
    board_power_init();
    adc_vbg_init();
...
{
#if (TCFG_AUDIO_MIC_MODE == MIC_MODE_DIFF_END)
	audio_mic_ldo_en(en, &adc_data);//差分麦默认使用mic_bias1(PB7)供电
#endif
}
//maskrom 使用到的io
...
void board_set_soft_poweroff(void)
{
	...
	if(mode == 3){
		...
		if(spi_get_port() == 0){
			porta_value = 0xffff;
			portb_value = 0xffff & (~(BIT(1)|BIT(4)));
			portc_value = 0xffff & (~BIT(3));
		} else {
			porta_value = 0xffff & (~BIT(7));
			portb_value = 0xffff & (~BIT(1));
			portc_value = 0xffff & (~BIT(5));
		}
	}
