#ifndef CONFIG_BOARD_WIRELESS_DUPLEX_EARPHONE_CFG_H
#define CONFIG_BOARD_WIRELESS_DUPLEX_EARPHONE_CFG_H

#ifdef CONFIG_BOARD_WIRELESS_DUPLEX_EARPHONE

#include "board_wireless_duplex_earphone_global_build_cfg.h"
...
//编解码采样率如果有修改，请对应修改dongle端的编解码采样率，earphone的编码要对应dongle的解码，earphone的解码要对应dongle的编码
#define WIRELESS_CODING_SAMPLERATE		(16000)//(44100)//
...
//游戏耳机切换到蓝牙模式使能,需要使用8M flash
#define TCFG_DUPLEX_EARPHONE_MODE_SW		DISABLE
 ...
 * */
#define TCFG_AUDIO_MIC_MODE                 MIC_MODE_DIFF_END	//MIC_MODE_SINGLE_END
#else
...
#define TCFG_MUSIC_EQ_ENABLE				DISABLE_THIS_MOUDLE
...
#endif
#endif
//由于697N资源有限，双工SDK，打开在线调试时，使用上行eq需要关闭下行数据流,使用下行eq需要关闭上行数据流
#if TCFG_EQ_ONLINE_ENABLE
#if TCFG_PHONE_EQ_ENABLE
#define WIRELESS_DOWNSTREAM_ENABLE			DISABLE
#elif TCFG_MUSIC_EQ_ENABLE
#define WIRELESS_UPSTREAM_ENABLE			DISABLE
#endif
#endif
//双工SDK，697n资源不够，不支持上下行同时打开
#if TCFG_PHONE_EQ_ENABLE&&TCFG_MUSIC_EQ_ENABLE
#error "no support phone eq and music eq at the same time"
#endif
//*********************************************************************************//
//                          新音箱配置工具 && 调音工具                             //
//*********************************************************************************//
...
#define TRANS_MULTI_BLE_MASTER_NUMS       0 /*配置主机个数*/
#define TRANS_MULTI_BLE_SM_ENABLE         0 /*配置加密 en*/
...
#endif
//<!信号不好，高延时门限
#define WL_HIGH_DELAY_LIMIT             WL_LOW_DELAY_LIMIT+12
...
#if !TCFG_EFFECT_TOOL_ENABLE && TCFG_EQ_ONLINE_ENABLE || TCFG_EFFECT_TOOL_ENABLE && !TCFG_EQ_ONLINE_ENABLE
#error  "需使能两者TCFG_EFFECT_TOOL_ENABLE && TCFG_EQ_ONLINE_ENABLE"
#endif
//*********************************************************************************//
//                                 配置结束                                         //
//*********************************************************************************//
...
#endif //CONFIG_BOARD_WIRELESS_DUPLEX_EARPHONE
#endif //CONFIG_BOARD_WIRELESS_DUPLEX_EARPHONE_CFG_H
