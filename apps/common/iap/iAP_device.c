...
{
    u32 wlen, len;
    printf("iAP_bulk_in_process: %d\n", iAP_var_wLength);
    iAP_printf_buf(iAP_var_pTxBuf, iAP_var_wLength);
#if 1
    wlen = 0;
    while (1) {
        len = iAP_var_wLength - wlen;
        len = (len > 64) ? 64 : len;
        if (MCU_SRAMToUSB_app(iap_usb_hdl, iAP_var_pTxBuf + wlen, len) != len) {
            break;
        }
        wlen += len;
        if (wlen >= iAP_var_wLength) {
            break;
        }
    }
    return wlen;
...
u8 apple_mfi_link(void *hdl)
{
    ...
    if (IAP2_VERSIONS == iAP_support()) {
        ...
        if (IAP2_LINK_SUCC == res) {
            ...
            iAP_deg_str("iAP2_link succ!\n");
            iAP2_power_to_apple_PowerStart();
        }
        return res;
    } else {
        os_time_dly(100);
