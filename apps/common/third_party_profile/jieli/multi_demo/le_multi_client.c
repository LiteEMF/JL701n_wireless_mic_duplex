...
void user_client_report_data_callback(att_data_report_t *report_data)
{
    ...
    if ((s8) - 1 == mul_get_dev_index(report_data->conn_handle, MULTI_ROLE_CLIENT)) {
...
{
    s8 tmp_cid = mul_get_dev_index(conn_handle, MULTI_ROLE_CLIENT);
...
static void client_report_adv_data(adv_report_t *report_pt, u16 len)
{
    ...
    if (!find_tag) {
        ...
        if (NULL == ll_link_for_address(report_pt->address_type, report_pt->address)) {
            ...
            r_printf("The device link is not disconnected");
        }
    }
...
static u16 create_connection_timeout_tid = 0;
static void create_connection_timeout_handler()
{
    int ret = 1;
    if (create_connection_timeout_tid) {
        create_connection_timeout_tid = 0;
        ret = client_create_cannel_api();
        if (!ret) {
...
static void client_create_connection(u8 *conn_addr, u8 addr_type)
{
...
#if((APP_MAIN == APP_WIRELESS_MIC_2T1) && (WIRELESS_BLE_CONNECT_INTERVAL == 2))
    if (0) {
#else
    ...
    if (!create_connection_timeout_tid) {
        create_connection_timeout_tid = sys_timeout_add(NULL, create_connection_timeout_handler, 4000);
    }
}
...
static void can_send_now_wakeup(void)
{
    ...
    if (!mul_dev_have_connected(MULTI_ROLE_CLIENT)) {
        return;
    }
    ...
    case HCI_EVENT_PACKET:
        ...
        case HCI_EVENT_LE_META:
            ...
            case HCI_SUBEVENT_LE_CONNECTION_COMPLETE: {
                ...
                if (create_connection_timeout_tid) {
                    sys_timeout_del(create_connection_timeout_tid);
                    create_connection_timeout_tid = 0;
                }
                if (packet[3]) {
                    log_info("LE_MASTER CREATE CONNECTION FAIL!!! %0x\n", packet[3]);
...
int client_2t1_write_send(u16 con_handle, u8 *data, u16 len)
{
    if (!con_handle) {
        return APP_BLE_OPERATION_ERROR;
    }

    if (con_handle != client_con_handle[con_handle - 0x50]) {
        return APP_BLE_OPERATION_ERROR;
    }
    ...
    return ble_op_multi_att_send_data(con_handle, 0x0013, data, len, ATT_OP_WRITE);
}
...
void client_reset_work_curr_state(void)
{
#ifdef CONFIG_ADAPTER_ENABLE
