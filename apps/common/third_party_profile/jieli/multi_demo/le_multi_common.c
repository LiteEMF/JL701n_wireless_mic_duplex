...
#define MULTI_ATT_SEND_CBUF_SIZE        (512*2)                 //
...
#define MULTI_TCFG_BLE_SECURITY_EN          0/*是否发请求加密命令*/
void ble_profile_init(void)
{
    ...
    if (config_le_sm_support_enable) {
        ...
        ble_sm_setup_init(IO_CAPABILITY_DISPLAY_ONLY, SM_AUTHREQ_MITM_PROTECTION | SM_AUTHREQ_BONDING, 7, MULTI_TCFG_BLE_SECURITY_EN);
#else
        ble_sm_setup_init(IO_CAPABILITY_NO_INPUT_NO_OUTPUT, SM_AUTHREQ_MITM_PROTECTION | SM_AUTHREQ_BONDING, 7, MULTI_TCFG_BLE_SECURITY_EN);
#endif
    }
    ...
    extern void wlm_rx_testox_comm_init(void *evt_cbk);
    wlm_rx_testox_comm_init(&cbk_packet_handler);
#else
...
void bt_ble_init(void)
{
    ...
    if (config_btctler_le_master_multilink) {
