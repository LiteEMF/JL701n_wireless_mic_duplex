...
#if USB_EP_PROTECT
#define     AUDIO_DMA_SIZE  1024+192
#else
#define     AUDIO_DMA_SIZE  192+192
#endif
#else
...
#if (USB_SOF_FILL_DATA_MODE)
    if (usb_interrupt_tx[usb_id][0] != NULL) {
        struct usb_device_t *usb_device = usb_id2device(usb_id);
        usb_interrupt_tx[usb_id][0](usb_device, 0);
    }
#else
