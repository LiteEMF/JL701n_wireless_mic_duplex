...
error_t hal_os_timer_register(api_os_timer_t *ptimer)
{
	if(TIMER_ONE_SHOT & ptimer->ctrl){
		ptimer->os_time_id = sys_s_hi_timerout_add(ptimer->pa, ptimer->cb, ptimer->timeout);
	}else{
		ptimer->os_time_id = sys_s_hi_timer_add(ptimer->pa, ptimer->cb, ptimer->timeout);
	}
...
error_t hal_os_timer_delete(api_os_timer_t *ptimer)
{
	...
	if(TIMER_ONE_SHOT & ptimer->ctrl){
		sys_s_hi_timeout_del(ptimer->os_time_id);
	}else{
		sys_s_hi_timer_del(ptimer->os_time_id);
	}
	return err;
}
...
error_t hal_os_timer_modify(api_os_timer_t *ptimer)
{
	...
	if(TIMER_ONE_SHOT & ptimer->ctrl){
		sys_s_hi_timeout_modify(ptimer->os_time_id,ptimer->timeout);
	}else{
		sys_s_hi_timer_modify(ptimer->os_time_id,ptimer->timeout);
	}
