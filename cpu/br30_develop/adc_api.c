...
static u16 vbat_voltage_array[PMU_CH_VALUE_ARRAY_SIZE];
...
void adc_scan(void *priv)
{
    ...
    if (adc_sample_flag) {
        if (adc_sample_flag == 2) {
            ...
            return;
        } else if (adc_sample_flag == 4) {
            vbat_adc_res = _adc_res;
            vbat_voltage = adc_value_to_voltage(vbg_adc_res, vbat_adc_res);
            adc_value_push(vbat_voltage_array, vbat_voltage);
...
void adc_reset(void)
{
    ...
    vbat_voltage = adc_value_to_voltage(adc_value_avg(vbg_value_array), adc_value_avg(vbat_value_array));
    printf("vbat = %d mv\n", vbat_voltage * 4);
    adc_value_push(vbat_voltage_array, vbat_voltage);
...
void vdd_trim()
{
    ...
    u8 irq_adc = irq_read(IRQ_SARADC_IDX);
    bit_clr_ie(IRQ_SARADC_IDX);
    ...
    bit_set_ie(IRQ_SARADC_IDX);
}
#define ADC_SCAN_NUM    16
__attribute__((noinline))
AT(.volatile_ram_code)
static u32 get_vddio_voltage()
{
...
static iovdd_voltage  iovdd_voltage_lev;
u8 get_miovdd_trim_level()
{
    return iovdd_voltage_lev.miovdd_lev;
}
u8 get_wiovdd_trim_level()
{
    return iovdd_voltage_lev.wiovdd_lev;
}
...
u32 iovdd_trim(trim)
{
    ...
    if (trim) {
        ...
        if (check_vbat < 3700) {
            iovdd_voltage_lev.wiovdd_lev = m_iovdd_lev;
            iovdd_voltage_lev.miovdd_lev = m_iovdd_lev;
            printf("check_vbat = %d\n", check_vbat);
            return 0;
        }
        ...
        iovdd_voltage_lev.miovdd_lev = mlev;
        ...
        iovdd_voltage_lev.wiovdd_lev = wlev;
        ...
        for (u8 i = 0; i < 8; i++) {
            printf("adc_miovdd_voltage = %d   adc_wiovdd_voltage = %d\n", adc_miovdd_voltage[i], adc_wiovdd_voltage[i]);
...
u32 check_iovdd_trim()
{
    if (sizeof(iovdd_voltage_lev) == syscfg_read(IO_VDD_TRIM, &iovdd_voltage_lev, sizeof(iovdd_voltage_lev))) {
        return 0;
    } else {
        memset(&iovdd_voltage_lev, 0xff, sizeof(iovdd_voltage_lev));
        return 1;
    }
}
void adc_init()
{
    ...
    trim = check_iovdd_trim();
    trim = iovdd_trim(trim);
    if (trim) {
        syscfg_write(IO_VDD_TRIM, &iovdd_voltage_lev, sizeof(iovdd_voltage_lev));
        /* printf_buf(&iovdd_voltage_lev,sizeof(iovdd_voltage_lev)); */
    }
    u8 miovdd_lev = get_miovdd_trim_level();
    u8 wiovdd_lev = get_wiovdd_trim_level();
    printf("miovdd_lev = %d\n", miovdd_lev);
    printf("wiovdd_lev = %d\n", wiovdd_lev);
